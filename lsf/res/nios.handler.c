...
doAcceptResCallback_(int s, struct niosConnect *connReq)
{
    ...
    if (readDecodeHdr_(newsock, buf, NB_SOCK_READ_FIX, &xdrs, &msgHdr) < 0) {
        close(newsock);
        return(-1);
    }
    ...
    if (readDecodeMsg_(newsock, buf, &msgHdr, NB_SOCK_READ_FIX, &xdrs,
                      (char *) connReq, xdr_niosConnect, NULL) < 0) {   

...
placeReq(XDR *xdrs, struct sockaddr_in *from, struct LSFHeader *reqHdr, int s)
{
    ...
    int  ncandidates, ncandidateInst;
    int ignore_res, propt, returnCode;
    char clName;
    char *replyStruct;
    ...
    placeReply.placeInfo = (struct placeInfo *) calloc( placeReply.numHosts,
...
static
void potentialOfHost(struct hostNode *hPtr, struct resVal *resValPtr)
{
...
selectBestInstances(int ncandidates, int needed, char locality,
			    int ignore_res)
{
    static char fname[] = "selectBestInstances()";
    int avail,additional;
    int i;

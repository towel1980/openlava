...
{
    loadIndex = (struct loadIndexLog *)
	bhist_malloc(sizeof(struct loadIndexLog));
    loadIndex->nIdx = 11;
...
char *timeStampStr, int option)
{
    ...
    char   tBuff[20];
    char   *disptime = NULL;
    char   prline[MSGSIZE], *hostPtr, *cp, *cp1;
    int display;
    int lastChkPeriod;
	...
	if (timeStamp == 0 )
        ...
        job->exHosts = (char **)
            bhist_calloc(job->numExHosts, sizeof(char *));
        for (i=0; i<job->numExHosts; i++) {
            job->exHosts[i] = (char *) bhist_malloc(MAXHOSTNAMELEN);
...
initJobIdIndexS( struct jobIdIndexS *indexS, char *fileName )
{
	...
	if (errno != ENOENT) {
	    fprintf(stderr, I18N(3172, 
		"failed to open the jobId index file.\n")); /* catgets 3172 */
	    perror(fileName);
	}
	return(-1);
    }
    ...
    if ((fscanf(indexS->fp, "%s %s %d %ld",
        tag, version, &(indexS->totalRows), &(indexS->lastUpdate)) != 4) ||
	(strcmp(tag, LSF_JOBIDINDEX_FILETAG))) {
	fprintf(stderr, I18N(3173, 
	    "wrong jobId index file format.\n")); /* catgets 3173 */
	fclose(indexS->fp);
	return(-1);
    }

...
char *
bhist_calloc(int nelem, int esize)
{
    char *sp;

    if (nelem <= 0) {
	fprintf(stderr, (_i18n_msg_get(ls_catd,NL_SETN,3451, "Warning: calloc %d elements; allocating 1 element\n")), /* catgets  3451  */
		nelem);
	nelem = 1;
    }
    sp = (char *) calloc(nelem, esize);
...
char *
bhist_malloc(int len)
{
    char *sp;

    if (len <= 0) {
	fprintf(stderr, (_i18n_msg_get(ls_catd,NL_SETN,3453, "Warning: malloc %d bytes; allocating %d bytes\n")), /* catgets  3453  */
		len, sizeof(int));
	len = sizeof(int);
    }
    sp = malloc(len);
...
initJobList (void)        
{
    ...
    q = (struct jobRecord *) bhist_malloc(sizeof (struct jobRecord));
    q->forw = q->back = q;
    return q;
...
read_newjob(struct eventRec *log)
{
    ...
    if (submitPtr->numAskedHosts > 0)
        submitPtr->askedHosts = (char **) bhist_calloc
                                  (submitPtr->numAskedHosts, sizeof (char *));
    for (i=0;i<submitPtr->numAskedHosts;i++) {
        submitPtr->askedHosts[i] = (char *) bhist_malloc( MAXHOSTNAMELEN);
    ...
    if (submitPtr->nxf > 0) {
	submitPtr->xf = (struct xFile *) bhist_calloc(submitPtr->nxf,
						   sizeof(struct xFile));
...
read_startjob(struct eventRec *log)
{
	...
	jobRecord->job->exHosts = (char **) 
	    bhist_calloc(jobRecord->job->numExHosts, sizeof(char *));
    
        for (i=0; i<jobRecord->job->numExHosts; i++) {
	    jobRecord->job->exHosts[i] = (char *) bhist_malloc(MAXHOSTNAMELEN);
...
read_newstat(struct eventRec *log)
{
    ...
    event = (struct eventRecord *) bhist_malloc( sizeof(struct eventRecord));
...
read_sigact(struct eventRec *log)
{
    ...
    event->sigSymbol = (char *)
                bhist_malloc(strlen(sigactLog->signalSymbol) + 1);
...
read_mig(struct eventRec *log)
{
	...
	event->migAskedHosts = (char **) bhist_calloc(event->migNumAskedHosts,
						      sizeof(char *));
	for (i = 0; i < event->migNumAskedHosts; i++) {
	    event->migAskedHosts[i] = (char *)
		bhist_malloc(strlen(migLog->askedHosts[i]) + 1);
...
read_signal(struct eventRec *log)
{
    ...
    event->sigSymbol = (char *)
                bhist_malloc(strlen(log->eventLog.signalLog.signalSymbol) + 1);
...
read_jobmsg(struct eventRec *log)
{
    ...
    event->jmSrc = (char *)
                bhist_malloc(strlen(log->eventLog.jobMsgLog.src) + 1);
    strcpy(event->jmSrc,  log->eventLog.jobMsgLog.src);
    event->jmDest = (char *)
                bhist_malloc(strlen(log->eventLog.jobMsgLog.dest) + 1);
    strcpy(event->jmDest,  log->eventLog.jobMsgLog.dest);
    event->jmMsg = (char *)
                bhist_malloc(strlen(log->eventLog.jobMsgLog.msg) + 1);
...
read_jobmsgack(struct eventRec *log)
{
    ...
    event->jmSrc = (char *)
                bhist_malloc(strlen(log->eventLog.jobMsgAckLog.src) + 1);
    strcpy(event->jmSrc,  log->eventLog.jobMsgAckLog.src);
    event->jmDest = (char *)
                bhist_malloc(strlen(log->eventLog.jobMsgAckLog.dest) + 1);
    strcpy(event->jmDest,  log->eventLog.jobMsgAckLog.dest);
    event->jmMsg = (char *)
                bhist_malloc(strlen(log->eventLog.jobMsgAckLog.msg) + 1);
...
read_loadIndex(struct eventRec *log)
{
    ...
    loadIndex->name = (char **) bhist_calloc(loadIndex->nIdx, sizeof(char *));
    for (i = 0; i < loadIndex->nIdx; i++) {
        loadIndex->name[i] = (char *)bhist_malloc(MAXLSFNAMELEN);
...
read_jobforce(struct eventRec *log)
{
    ...
    event->userName = putstr_(log->eventLog.jobForceRequestLog.userName);
...
initJobInfo (void)
{
    ...
    job = (struct jobInfoEnt *) bhist_malloc (sizeof(struct jobInfoEnt));
    job->cpuTime = 0;
    job->cpuFactor = 0;
    ...
    submitPtr->queue       = (char *) bhist_malloc(MAX_LSB_NAME_LEN);
    submitPtr->jobName     = (char *) bhist_malloc(MAX_CMD_DESC_LEN);
    submitPtr->command     = (char *) bhist_malloc(MAX_CMD_DESC_LEN);
    submitPtr->resReq      = (char *) bhist_malloc(MAXLINELEN);
    submitPtr->inFile      = (char *) bhist_malloc(MAXFILENAMELEN);
    submitPtr->outFile     = (char *) bhist_malloc(MAXFILENAMELEN);
    submitPtr->errFile     = (char *) bhist_malloc(MAXFILENAMELEN);
    submitPtr->hostSpec    = (char *) bhist_malloc(MAXFILENAMELEN);
    submitPtr->chkpntDir   = (char *) bhist_malloc(MAXFILENAMELEN);
    submitPtr->dependCond  = (char *) bhist_malloc(6*MAXLINELEN);
    submitPtr->preExecCmd  = (char *) bhist_malloc(MAXLINELEN);
    submitPtr->mailUser  = (char *) bhist_malloc(MAXHOSTNAMELEN);
    submitPtr->projectName = (char *) bhist_malloc(MAX_LSB_NAME_LEN);
    submitPtr->loginShell = (char *) bhist_malloc(MAX_LSB_NAME_LEN);
    job->user     = (char *) bhist_malloc(MAX_LSB_NAME_LEN);
    job->fromHost = (char *) bhist_malloc(MAXHOSTNAMELEN);
    job->cwd      = (char *) bhist_malloc(MAXFILENAMELEN);
    job->execCwd      = (char *) bhist_malloc(MAXFILENAMELEN);
    job->execHome      = (char *) bhist_malloc(MAXFILENAMELEN);
    job->execUsername = (char *) bhist_malloc(MAX_LSB_NAME_LEN);
...
parse_event(struct eventRec *log, struct bhistReq *Req)
{
	...
	if (jobRecord != NULL) {
		...
		if (log->type == EVENT_JOB_EXECUTE) {
		    ...
		    event->execHome = (char *)bhist_malloc(MAXFILENAMELEN);
		    strcpy (event->execHome, jobRecord->job->execHome);
		    event->execCwd = (char *)bhist_malloc(MAXFILENAMELEN);
		    strcpy (event->execCwd, jobRecord->job->execCwd);
		    event->execUsername = 
				(char *)bhist_malloc(MAX_LSB_NAME_LEN);
		    strcpy (event->execUsername, 
				       jobRecord->job->execUsername);
                } else {
		    event->numExHosts = jobRecord->job->numExHosts;
                    if (event->numExHosts > 0) {
                        event->exHosts = (char **) bhist_calloc(event->numExHosts, sizeof(char *));
                        for (i=0; i<event->numExHosts; i++) {
                            event->exHosts[i] = (char *) bhist_malloc(MAXHOSTNAMELEN);

...
access:
    ...
    if (lock_fd >= 0) {
        ...
        close(lock_fd);
	chuser(batchId);
        ls_syslog(LOG_INFO, (_i18n_msg_get(ls_catd , NL_SETN, 8274, "%s: Got lock file")), fname);       /* catgets 8274 */
        gotLock = TRUE;
        return 0;
	 ...
	 chuser(batchId);
         if (fd < 0) {
             ls_syslog(LOG_ERR, _i18n_msg_get(ls_catd , NL_SETN, 8243,
		 "%s: Can't open existing lock file <%s>: %m"), /* catgets 8243 */
		 fname, lockfile);
             return(MASTER_FATAL);
         }
         i = 0;
         ...
         if (buf[i] == ':') {
             ...
             if (equalHost_(myhostnm, buf)) {
                 ...
                 pid = atoi(buf);
                 if ( kill(pid, 0) < 0) {
		     ls_syslog(LOG_ERR, _i18n_msg_get(ls_catd , NL_SETN, 8244,
			"%s: Last owner of lock file was on this host with pid <%d>; attempting to take over lock file"),fname,pid); /* catgets 8244 */
		     close(fd);
		     force = 1;
		     goto access;
		 }
...
releaseElogLock(void)
{
	...
	chuser(batchId);
        if (ul_val != 0) {
            ls_syslog(LOG_ERR, I18N_FUNC_S_FAIL_M, fname, "unlink", lockfile);
        } else
            ls_syslog(LOG_INFO, (_i18n_msg_get(ls_catd , NL_SETN, 8275, "%s: Released lock file")), "releaseElogLock");    /* catgets 8275 */
    }
}

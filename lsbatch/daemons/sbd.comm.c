...
getJobsState (struct sbdPackage *sbdPackage)
{
    ...
    if (! xdr_encodeMsg(&xdrs, NULL, &hdr, NULL, 0, NULL)) {
        ls_syslog(LOG_ERR, I18N_FUNC_FAIL_MM, fname, "xdr_encodeMsg");
        xdr_destroy(&xdrs);
            ...
            if (equalHost_(masterHost, myhostnm)) {
                ...
                if (debug)
                    ls_syslog(LOG_ERR, _i18n_msg_get(ls_catd , NL_SETN, 5210,
                                                     "%s: mbatchd on host <%s> not responding: %s"), fname, masterHost, lsb_sysmsg()); /* catgets 5210 */
        } else {
            break;                        
        }
    ...
    switch (reply) {
        case LSBE_NO_ERROR:
            ...
            if (!xdr_sbdPackage (&xdrs, sbdPackage, &hdr)) {
                ls_syslog(LOG_ERR, I18N_FUNC_S_FAIL_M, fname, "xdr_sbdPackage", 
                          masterHost);
            ...
            for (i = 0; i < numJobs; i++) {
                if (!xdr_arrayElement(&xdrs, (char *)&jobSpecs, &hdr, 
                                      xdr_jobSpecs)) {
                    ls_syslog(LOG_ERR, I18N_FUNC_S_FAIL, fname, "xdr_jobSpecs", 
                              masterHost);
        ...
        case LSBE_BAD_HOST:
            ls_syslog(LOG_ERR, _i18n_msg_get(ls_catd , NL_SETN, 5215,
                                             "%s: This host is not used by the batch system"), /* catgets 5215 */
                      fname);
            free(reply_buf);
            die(SLAVE_FATAL);
        ...
        case LSBE_PORT:
            ls_syslog(LOG_ERR, _i18n_msg_get(ls_catd , NL_SETN, 5216,
                                             "%s: mbatchd reports that it is using a bad port"), /* catgets 5216 */
                      fname);
            free(reply_buf);
            die(SLAVE_FATAL);
        ...
        default:
            ls_syslog(LOG_ERR, _i18n_msg_get(ls_catd , NL_SETN, 5217,
                                             "%s: Invalid return code <%d> from mbatchd on <%s>"),  /* catgets 5217 */
